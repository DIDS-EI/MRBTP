...
class MiniGridMAGridEnv(MiniGridEnv,Env):
    def __init__(
        self,
        num_agent: int = 1,
        **kwargs
    ...
    def reset(
        ...
        *,
        ...
        # Randomly place each agent on an empty cell with a random direction.
        # Works for any value of `num_agent` and respects the gym seed.
        placed: set = set()
        for agent in self.agents[:self.num_agent]:
            pos = self.place_obj(
                None,
                max_tries=1000,
                reject_fn=lambda env, p: tuple(p) in placed,
            )
            pos = tuple(pos)
            placed.add(pos)
            agent.position = pos
            agent.direction = self._rand_int(0, 4)
