...
class Agent(object):
    ...
    def planning_for_subgoal(self, subgoal):
        # NOTE: This method previously imported ``mabtpg.btp.pbtp.PBTP``, which
        # has not existed in the codebase for a long time, and the method is
        # not called from anywhere. It is kept as a stub so external callers
        # (if any) get a clear error instead of an ImportError on import.
        raise NotImplementedError(
            "Agent.planning_for_subgoal was removed during refactor; use "
            "MRBTP / MABTP / MAOBTP from mabtpg.btp instead."
        )
