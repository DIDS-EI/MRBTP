...
# All generated .bt / .svg / .png files go here (gitignored).
...
# ########################################
# NOTE: action_sequences 必须与 num_agent 一一对应。
# 当 num_agent=2 时，将 4 个复合动作分配给 2 个 agent。
action_sequences = [
    {
...
for agent_id in range(num_agent):
    ...
    for name, sub_btml in btml_list[agent_id].sub_btml_dict.items():
        ...
        tmp_bt.draw(file_name=f"{agent_id}-{name}", target_directory=str(OUTPUT_DIR))
...
for i in range(num_agent):
    ...
    bt_ls[i].save_btml(str(OUTPUT_DIR / f"robot-{i}.bt"))
    bt_ls[i].draw(file_name=f"robot-{i}", target_directory=str(OUTPUT_DIR))
